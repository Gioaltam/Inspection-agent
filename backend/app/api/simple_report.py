"""Simple report viewer that generates HTML from JSON"""
...
@router.get("/photo/{report_id}/{photo_filename}")
def get_photo_report_html(report_id: str, photo_filename: str):
    """Generate HTML view for a specific photo's analysis from JSON report"""
    try:
...
@router.get("/simple/{report_id}")
def view_simple_report(report_id: str):
    ...
    try:
        ...
        if not row:
            return HTMLResponse(content="<h1>Report not found</h1>")
        ...
        if json_path.exists():
            ...
            <head>
                ...
                <style>
                    ...
                    .item {{ border: 1px solid #444; padding: 10px; margin: 10px 0; border-radius: 5px; }}
                    .critical {{ border-color: red; }}
                    .important {{ border-color: orange; }}
                    h1 {{ color: #ef4444; }}
                </style>
            </head>
            <body>
            ...
            for item in data.get('items', []):
                ...
                # Build description from observations and issues
                description = ""
                if item.get('observations'):
                    description += "<strong>Observations:</strong><ul>"
                    for obs in item['observations']:
                        description += f"<li>{obs}</li>"
                    description += "</ul>"
                
                if item.get('potential_issues'):
                    description += "<strong>Issues:</strong><ul>"
                    for issue in item['potential_issues']:
                        description += f"<li>{issue}</li>"
                    description += "</ul>"
                
                if item.get('recommendations'):
                    description += "<strong>Recommendations:</strong><ul>"
                    for rec in item['recommendations']:
                        description += f"<li>{rec}</li>"
                    description += "</ul>"
                ...
                <div class="item {severity}">
                    <h3>{item.get('location', 'Unknown')}</h3>
            ...
            return HTMLResponse(content=f"<h1>JSON file not found at {json_path}</h1>")
